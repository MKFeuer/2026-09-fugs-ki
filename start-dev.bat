...
start "Tools MCP" /d "%~dp0tools" uv run python -c "from main import mcp; mcp.settings.port = 3002; mcp.run(transport='streamable-http')"

:: Start commandx MCP server (port 3003)
start "CommandX MCP" /d "%~dp0commandx" uv run python -c "from main import mcp; mcp.settings.port = 3003; mcp.run(transport='streamable-http')"
